...
- `main.go` — Entry point, CLI flag parsing, week range computation, orchestration. Flags: `--repo`, `--branch`, `--weeks`, `--output`, `--exclude`, `--stats-output`, `--html`, `--serve`, `--port`, `--min-prs`, `--exclude-bottom-contributor-pct`, `--stats-window-pct`.
...
- **Stats window**: Compares first N% vs last N% of valid weeks (min 1 week per side), configurable via `--stats-window-pct` (default 5). The `windowSize` is stored on `consolidatedRow` so the HTML can display actual date ranges.
